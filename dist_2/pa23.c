    ...
    for (int i = 0; i < X; i++) {
        ...
        switch (tmp) {
            case 0:
                ...
                archieve_balance_history(IO.balance_state.s_balance, beggining_time);
                child_routine(&IO);
                return 0;
            case -1:
                log_errors(1);
    ...
    for (int i = 0; i < X + 1; i++) {
        for (int j = 0; j < X + 1; j++) {
            if (i != j) {
                ...
                //TODO: F_GETFL
                fcntl(pipe_id[1], F_SETFL, O_NONBLOCK);
                temp = find_pipes_index(i, j);
                temp -> fd_write = pipe_id[1];
                log_pipes(1, i, j);

                fcntl(pipe_id[0], F_SETFL, O_NONBLOCK);
...
void system_start(self_data * IO){
    ...
    Message msg;
    int msg_length = sprintf(msg.s_payload, log_started_fmt, get_physical_time(), IO -> to, getpid(), getppid(), IO -> balance_state.s_balance);
...
void archieve_balance_history (int amount, timestamp_t current_time) {
    ...
    for (int i = 0; i < balance_history_ptr -> s_history_len; i++) {
        if (balance_history_ptr -> s_history[i].s_time == current_time) {
            ...
            balance_history_ptr -> s_history[i].s_balance_pending_in = 0;
            return;
        }
    }
...
void child_productive_work(self_data * IO) {
    ...
    while(done_received || !stop_flag) {
        ...
        while (my_time < t) {
            ...
            balance_history_ptr -> s_history[balance_history_ptr -> s_history_len].s_balance = IO->balance_state.s_balance;
            balance_history_ptr -> s_history[balance_history_ptr -> s_history_len].s_balance_pending_in = 0;
            balance_history_ptr -> s_history_len++;
            my_time ++;
        }
        ...
        if (receive_any(IO, &msg) == 0) {
            if (msg.s_header.s_type == TRANSFER) {
                ...
                if(IO -> from == PARENT_ID) {
                    ...
                    IO -> balance_state.s_time = get_physical_time();
                    
                    archieve_balance_history(IO->balance_state.s_balance, IO -> balance_state.s_time);
                ...
                archieve_balance_history(IO->balance_state.s_balance, get_physical_time());
                IO -> from = IO -> to;
                ...
                printf("Сообщения с заголовком %x я получать не должен\n", msg.s_header.s_type); //TODO: на инглише
                IO -> from = IO -> to;
            }
        }
    }
...
void system_done(self_data * IO) {
    Message msg;
    int msg_length = sprintf(msg.s_payload, log_done_fmt, get_physical_time(), IO -> to, IO -> balance_state.s_balance);
...
void log_events (self_data * IO, int log_num, TransferOrder * transfer_order) {
    ...
    switch (log_num) {
        case 1:
            sprintf(buffer, log_started_fmt, get_physical_time(), IO -> to, getpid(), getppid(), IO -> balance_state.s_balance);
            break;
        case 2:
            sprintf(buffer, log_received_all_started_fmt, get_physical_time(),  IO -> to);
            break;
        case 3:
            sprintf(buffer, log_done_fmt, get_physical_time(), IO -> to, IO -> balance_state.s_balance);
            break;
        case 4:
            sprintf(buffer, log_received_all_done_fmt, get_physical_time(), IO -> to);
            break;
        case 5:
            sprintf(buffer, log_transfer_out_fmt, get_physical_time(), transfer_order -> s_src, transfer_order -> s_amount,transfer_order -> s_dst);
            break;
        case 6:
            sprintf(buffer, log_transfer_in_fmt, get_physical_time(), transfer_order -> s_dst, transfer_order -> s_amount, transfer_order -> s_src);
        default:
            break;
    }
    printf(buffer, 0);
